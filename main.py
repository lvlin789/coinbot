...
def get_best_bid_ask() -> tuple[float, float]:
    ...
    ob, _ = coinone_client_in.get_orderbook(quote_currency=QUOTE, target_currency=CURRENCY, size=5)
    bids = ob.get("bids") or []
    asks = ob.get("asks") or []
...
@rate_limiter(40)
def fetch_balances(client: CoinoneClient) -> dict:
    """从远端私有代理获取余额，返回形如 {"BTC": 0.01, ...} 的字典（仅非零）。"""
    try:
        data, _ = client.get_balance_all()
        out: dict[str, float] = {}
...
def place_limit_order(client: CoinoneClient, side: str, price: str = None, qty: str = None, amount: str = None, type_: str = "LIMIT") -> str | None:
    """通过远端代理下限价单（post_only 为 False，便于尽快撮合）。返回 order_id 或 None。"""
    try:
        resp, _ = client.place_order(
        ...
        console.print(f"[red]下单失败: {resp}[/red]")
        return None
...
def transfer():
    ...
    while True:
        ...
        try:
            # 1) 读取双方余额
            bal_in = fetch_balances(coinone_client_in)
            bal_out = fetch_balances(coinone_client_out)
            amt_in = float(bal_in.get(CURRENCY, 0.0))
            amt_out = float(bal_out.get(CURRENCY, 0.0))
            console.print(f"[cyan]收款账号{CURRENCY}：{amt_in} ｜ 提币账号{CURRENCY}：{amt_out}[/cyan]")

            # 2) 获取买一/卖一用于定价
            bid, ask = get_best_bid_ask()
            console.print(f"[cyan]订单簿{CURRENCY} | 卖: {ask} 买: {bid}[/cyan]")
            

            # 3) 依据谁余额多决定方向（仅演示价格与对称数量逻辑）
            if amt_in > amt_out and amt_in > 0:
